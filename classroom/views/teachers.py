...
from ..forms import (BaseAnswerInlineFormSet, CourseAddForm, FileAddForm,
                     ...
                     QuestionForm, TeacherProfileForm)
from ..models import (ClassAnswer, Course, MyFile, Lesson, ClassQuestion, ClassQuiz,
                      StudentAnswer, TakenCourse, TakenQuiz, UserLog)
...
#         messages.success(
#             self.request, 'Your successfully changed your password!')
#         return super().form_valid(form)

#     def get_context_data(self, **kwargs):
#         """enrollment_request_count is used for base.html's navbar."""
#         kwargs['enrollment_request_count'] = get_enrollment_requests_count(
#             self.request.user)
...
class CourseCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        course.save()
        Rating.objects.create(count=0, total=0, average=0,
                              object_id=course.pk, content_type_id=15)
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['courses'] = self.request.user.courses \
            ...
            .annotate(taken_count=Count('taken_courses',
                                        filter=Q(
                                            taken_courses__status='enrolled'),
                                        distinct=True)) \
            .order_by('title')
...
class CourseUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'{course.title} has been successfully updated.')
        return redirect('teachers:course_change_list')
...
class QuizListView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = ClassQuiz.objects.filter(course__owner=self.request.user) \
            .exclude(course__status='deleted') \
...
class QuizResultsView(DetailView):
    ...
    def get_queryset(self):
        return ClassQuiz.objects.filter(course__in=Course.objects.filter(owner=self.request.user))
...
def accept_enrollment(request, taken_course_pk):
    ...
    TakenCourse.objects.filter(
        id=taken_course_get.pk).update(status='enrolled')
...
def add_files(request):
    if request.method == 'POST':
        form = FileAddForm(request.user, data=request.POST,
                           files=request.FILES)
        success = None
        if form.is_valid():
            for file in request.FILES.getlist('file'):
                ...
                if extension == '.pdf' or \
                    ...
                    MyFile.objects.create(
                        file=file, course=form.cleaned_data['course'], owner=request.user)
                    success = True
            ...
            if success:
                UserLog.objects.create(action='Uploaded file/s',
                                       ...
                                       user=request.user)
                messages.success(
                    request, 'The files were successfully uploaded.')
                return redirect('teachers:file_list')
            else:
                messages.error(
                    request, 'The only allowed file formats are .pdf, .doc, .docx, .ppt, and .pptx.')
...
def add_question(request, course_pk, quiz_pk):
    ...
    quiz = get_object_or_404(ClassQuiz, pk=quiz_pk, course=course)
...
def delete_file(request, file_pk):
    ...
    file_name = MyFile.objects.values_list(
        'file', flat=True).get(id=file_get.pk)
...
def delete_question(request, course_pk, quiz_pk, question_pk):
    teacher = request.user
    quiz_get = get_object_or_404(ClassQuiz, pk=quiz_pk)
    question_get = get_object_or_404(ClassQuestion, pk=question_pk)
    quiz = ClassQuiz.objects.get(id=quiz_get.pk, course__owner=teacher)
    ClassQuestion.objects.get(id=question_get.pk, quiz=quiz).delete()
...
def delete_quiz(request, quiz_pk):
    teacher = request.user
    quiz_get = get_object_or_404(ClassQuiz, pk=quiz_pk)
    ClassQuiz.objects.filter(id=quiz_get.pk, course__owner=teacher).delete()
...
def delete_quiz_from_list(request, quiz_pk):
    quiz_get = get_object_or_404(ClassQuiz, pk=quiz_pk)
    ClassQuiz.objects.filter(id=quiz_pk, course__owner=request.user).delete()
...
def edit_question(request, course_pk, quiz_pk, question_pk):
    course = get_object_or_404(Course, pk=course_pk, owner=request.user)
    quiz = get_object_or_404(ClassQuiz, pk=quiz_pk, course=course)
    question = get_object_or_404(ClassQuestion, pk=question_pk, quiz=quiz)

    AnswerFormSet = inlineformset_factory(
        ClassQuestion,  # parent model
        ClassAnswer,  # base model
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            messages.success(
                request, 'Question and answers are successfully saved!')
...
def edit_quiz(request, course_pk, quiz_pk):
    course = get_object_or_404(Course, pk=course_pk, owner=request.user)
    quiz = get_object_or_404(ClassQuiz, pk=quiz_pk, course=course)
    question = ClassQuestion.objects.filter(
        quiz=quiz).annotate(answers_count=Count('answers'))
...
def quiz_result_detail(request, quiz_pk, student_pk, taken_pk):
    quiz = get_object_or_404(ClassQuiz, pk=quiz_pk)
    questions = ClassQuestion.objects.filter(quiz=quiz)
    taken_quiz = get_object_or_404(
        TakenQuiz, pk=taken_pk, quiz=quiz, student_id=student_pk)
    ...
    answers = ClassAnswer.objects.filter(question__in=questions)
...
def reject_enrollment(request, taken_course_pk):
    ...
    messages.success(
        request, 'The student\'s request has been successfully rejected.')
    return redirect('teachers:enrollment_requests_list')
