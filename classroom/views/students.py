...
from ..forms import (StudentInterestsForm, StudentProfileForm, TakeQuizForm)
from ..models import (ClassAnswer, Course, Lesson, MyFile, ClassQuiz, ClassQuestion,
                      Student, StudentAnswer, TakenCourse, TakenQuiz, UserLog)
...
class StudentInterestsView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, 'Your interests are successfully updated!')
        return super().form_valid(form)
...
class TakenQuizListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        kwargs['grade'] = TakenQuiz.objects.filter(
            student_id=self.request.user.pk).aggregate(average=Avg('score'))
...
def enroll(request, pk):
    ...
    messages.info(
        request, 'You have successfully sent an enrollment request to the teacher in charge.')
...
def file_view(request, pk):
    ...
    try:
        ...
        response = FileResponse(
            open(f'media/{file_path}', 'rb'), content_type=file_type)
...
def take_quiz(request, course_pk, quiz_pk):
    ...
    if total_questions == 0:
        messages.error(
            request, 'We\'re sorry, there are currently no questions available for that quiz.')
        return redirect('course_details', course_pk)
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    ...
                    quiz_count = ClassQuiz.objects.filter(course_id=course_pk) \
                        .values_list('id', flat=True).count()

                    # Check if the taken quizzes and quizzes have equal count:
                    if taken_quiz_count == quiz_count:
                        TakenCourse.objects.filter(
                            course_id=course_pk, student_id=request.user.pk).update(status='finished')
...
def taken_quiz_result(request, taken_pk, quiz_pk):
    ...
    taken_quiz = get_object_or_404(
        TakenQuiz, pk=taken_pk, quiz=quiz, student_id=request.user.pk)
