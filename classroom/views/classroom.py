...
from ..forms import SearchCourses
from ..models import (Course, Lesson, MyFile, ClassQuiz, Student,
...
class CourseDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['lessons'] = Lesson.objects.select_related('quizzes') \
            ...
            .order_by('number')
        kwargs['quizzes'] = ClassQuiz.objects.filter(course_id=self.kwargs['pk']) \
            .order_by('lesson__number')
        kwargs['files'] = MyFile.objects.filter(course_id=self.kwargs['pk']) \
            .order_by('file')

        if self.request.user.is_authenticated:
            if self.request.user.is_taker:
                teacher = None
                ...
                quiz_count = ClassQuiz.objects.filter(course_id=self.kwargs['pk']) \
                    .values_list('id', flat=True).count()

                kwargs['progress'] = (
                    taken_quiz_count / quiz_count) * 100 if quiz_count != 0 else 0

                subject_interests = Student.objects.values_list(
                    'interests').filter(pk=self.request.user)
            ...
            elif self.request.user.is_master:
                student = None
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                UserLog.objects.create(action=f'Searched for "{query}"',
                                       ...
                                       user=request.user)
                enrollment_requests_count = get_enrollment_requests_count(
                    request.user)
    else:
        form = SearchCourses()
...
def class_index(request):
    if request.user.is_authenticated:
        ...
        context = {
            ...
            'title': 'Learn Forex and Crypto trading'
        }
