...
class SubjectCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        subject.save()
        messages.success(
            self.request, 'The subject has been successfully created!')
...
class StudentListView(ListView):
    ...
    def get_queryset(self):
        ...
        return User.objects.filter(is_taker=True, is_active=True) \
            .order_by('username')
...
class SubjectUpdateView(UpdateView):
    ...
    def get_success_url(self):
        ...
        messages.success(
            self.request, f'{title} has been successfully updated.')
...
class TeacherListView(ListView):
    ...
    def get_queryset(self):
        ...
        return User.objects.filter(is_master=True, is_active=True) \
            .order_by('username')
...
def dashboard(request):
    context = {
        ...
        'teachers_count': User.objects.values_list('id', flat=True)
                                      .filter(is_master=True, is_active=True).count(),
        'quizzes_count': ClassQuiz.objects.values_list('id', flat=True)
...
def deactivate_admin(request, pk):
    ...
    messages.success(
        request, 'The admin account has been successfully deleted.')
    return redirect('staff:admin_list')
...
def deactivate_student(request, pk):
    ...
    messages.success(
        request, 'The student account has been successfully deleted.')
...
def deactivate_teacher(request, pk):
    ...
    messages.success(
        request, 'The teacher account has been successfully deleted.')
...
def get_user_activities(request):
    select_data = {
        "action_date": "strftime('%%m-%%d-%%Y', datetime(created_at, 'localtime'))"}
